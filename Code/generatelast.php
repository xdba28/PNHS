<?php
  ...
  if(isset($_POST['generate'])){
							...
							$newnew = mysqli_query($mysqli,"SELECT count(pims_personnel.emp_No) as pers FROM pims_personnel, prs_salary_record, prs_salary, prs_grade
								...
								$timecheck = mysqli_query($mysqli,"SELECT count(id) as lodi FROM `prs_dtr_rec` WHERE month(date_import)= '$prevmonth' and year(date_import) = '$prevyear'") or die("timecheck error");
	...
	if ($ploc == $num  && $addinfonum == $num  )
	{
		...
		$qry9= mysqli_query($mysqli,"SELECT * FROM pims_personnel, prs_report 
										Where month(prs_report.date_issued) = '$prevmonth' AND
										year(prs_report.date_issued) = '$prevyear' AND
		...
		while($emprow=mysqli_fetch_assoc($emp)){
			...
			//
							...
							$qry6=mysqli_query($mysqli,"SELECT SUM(prs_loan.loan_amount) as total_amortization, loan_name,  date_start, date_end FROM pims_personnel, prs_loan
										...
										");
										$total_loan=mysql_fetch_assoc($qry6);
			...
			// INSERT ABSENT COST					
					...
					AND month(prs_dtr_rec.date_import) = '$prevmonth'
					AND year(prs_dtr_rec.date_import) = $prevyear'		
		...
		//DETERMINING IF LOAN IS PAID OR NOT || UPDATE LOAN	 
				...
				while($row4=mysqli_fetch_array($qry4))
				{
					...
					$stat = 'ONGOING';
					$stat1 = 'PAID';
					$prevmonthforloan = date('m-Y', strtotime('-1 months'));
						
					if($status == $stat)
					{
						
						if($prevmonthforloan == $date_end)
						{
										...
										");
											...
											$prevmonthpayloan = date('Y-m-d', strtotime('-1 months'));
						...
						$previousmonth = date('Y-m-d', strtotime('-1 months'));
						
				
					$qry8 = mysqli_query($mysqli," INSERT INTO prs_report (rep_basic_pay, rep_allowance, rep_gross_pay, rep_net_pay, date_issued, total_ph, rep_pi_total, total_gsis, absent_cost, total_tax, total_deduction, emp_No, report_sign) VALUES ('$salary', '$allowance', '$gross_pay', '$net_pay', '$previousmonth' ,'$ph_share', '$pi_total', '$total_gsis','$absent_cost' ,'$tax_total', '$deduc',  '$emp_No', 'LastMonthSign')
										") or die("Failed  Query insertreportlastmonth");
		
			echo "<script>  window.open('paytableB.php','_newtab')</script>";
			echo "<script> window.location='payrollreplastmonth.php' </script>";
