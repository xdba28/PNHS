 ...
 include('../sanitise.php');
		...
		if(isset($_POST['generate'])){
							$newnew = mysqli_query($mysqli,"SELECT count(pims_personnel.emp_No) as pers FROM pims_personnel, prs_salary_record, prs_salary, prs_grade
								...
								$timecheck = mysqli_query($mysqli,"SELECT count(id) as lodi FROM `prs_dtr_rec` WHERE month(date_import)= month(now()) and year(date_import) =year(now());");
	...
	if ($ploc == $num  && $addinfonum == $num  )
	{
		...
		$qry9= ("SELECT * FROM pims_personnel, prs_report 
		...
		while($emprow=mysqli_fetch_assoc($emp)){
			...
			// INSERT ABSENT COST					
					...
					$initial_total= $gross_pay - ($pi_total + $ph_share + $total_gsis +$loan);
		...
		$taxstatus = mysqli_query($mysqli,"SELECT * FROM prs_add_info, pims_personnel, prs_civil_status where pims_personnel.emp_No=prs_add_info.emp_No and prs_civil_status.civil_id=prs_add_info.civil_id and pims_personnel.emp_No= '$emp_No'");
			...
			while($taxstatusrow = mysqli_fetch_assoc($taxstatus))
				...
				elseif(($civil_status == 'Single' && $register == 'Not Registered') || ($civil_status== 'Married' && $register == 'Not Registered') )
					{
					$Z = mysqli_query($mysqli,"SELECT prs_tax_amount.amount_fr as amount , prs_stat_ex.percentage as percentage, prs_stat_ex.exemption as exemption from prs_stat_ex, prs_tax_amount, prs_withholding_tax
										where prs_stat_ex.stat_id=prs_tax_amount.stat_id and prs_withholding_tax.tax_id=prs_tax_amount.tax_id
		...
		//DETERMINING IF LOAN IS PAID OR NOT || UPDATE LOAN	 
				...
				while($row4=mysqli_fetch_array($qry4))
				{
					...
					if($status == $stat)
					{
						...
						if($date == $date_end)
						{
							//bring loan update in the last of the code.
							//bringout all the Insert out in this area code of loan.
							//
							...
							$paydateloan1 = date("Y-m-d");
							$insertpayloan = mysqli_query($mysqli,"INSERT INTO prs_loan_pay (pay_loan_name, pay_loan_amount, date_start, date_end, loan_status, pay_date, emp_No) VALUES ('$loan_name1', '$loan_amount1', '$date_start1', '$date_end1', '$loan_status1','$paydateloan1', '$emp_No')") or die("Failed  Query insertpayloan");
					...
					$dateget = date_default_timezone_set('Asia/Manila');
					$dateget = date("Y-m-d");
					
					$qry8 = mysqli_query($mysqli,"INSERT INTO prs_report (rep_basic_pay, rep_allowance, rep_gross_pay, rep_net_pay, date_issued, total_ph, rep_pi_total, total_gsis, absent_cost, total_tax, total_deduction, emp_No) VALUES ('$salary', '$allowance', '$gross_pay', '$net_pay', '$dateget', '$ph_share', '$pi_total', '$total_gsis','$absent_cost' ,'$tax_total', '$deduc',  '$emp_No')
										") or die("Failed  Query QRY8");								 
