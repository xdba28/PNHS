 ...
 include "connect.php";
    ...
    $fetch_roletype = mysqli_query($mysqli, "SELECT * FROM pims_personnel, pims_employment_records, pims_job_title
        ...
        $roletype = mysqli_fetch_array($fetch_roletype);
                    ...
                    header("Location: admin1/admin1_idx.php");


                }else{
                    echo "<script>
                        alert('Welcome!');
                        window.location='admin_redirect.php';
                    </script>"; 
                    header("Location: admin_redirect.php");
                    }
                }else if($arow['cms_account_type']=="user"){
                    session_start();
                    $_SESSION['user_data']['priv']['pms_priv']=$arow['pms_priv'];
                    $_SESSION['user_data']['acct']['emp_no']=$arow['emp_no'];
                    $_SESSION['user_data']['acct']['cms_account_ID']=$arow['cms_account_ID'];
                    ...
                    echo "<script>
                        alert('Welcome!');
                        window.location='USER/user_idx.php';
                    </script>";
                    header("Location: USER/user_idx.php");
                }
                else{
                    echo "<script>
                    alert('You are not allowed in this module!');
                    </script>";
