...
$pdf->SetLineWidth(.3);
	...
	$fetch_check=mysqli_query($mysqli,"SELECT payment_term, check_no FROM pms_po WHERE po_no ='".$po."'")
								or die (mysqli_error($mysqli));
	
	//$check_num = 0;
	$check_num = mysqli_fetch_array($fetch_check);
	//{
		if($check_num['payment_term'] == 'Check')
		{
			$term = $crow['payment_term']." - ".$crow['check_no'];
		}
		else if($check_num['payment_term'] == 'Cash')
		{
			$term = $crow['payment_term'];
		}
		else if($check_num['payment_term'] == 'Credit')
		{
			$term = $crow['payment_term'];
		}
	//}
    $pdf->Cell(163,5,"Mode of Procurement: ".$term,0,1,'R',true);
    $pdf->Ln();
    $pdf->SetFillColor(255, 255, 255);
    $pdf->SetTextColor(0);
