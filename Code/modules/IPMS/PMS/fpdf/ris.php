...
$pdf->SetLineWidth(.3);
	...
	$pdf->SetWidths(array(195));
	 ...
	 $purpose = mysqli_query($mysqli, "SELECT reason from pms_ris where pms_ris.ris_no = $ris")
                                      or die("Error: ".mysqli_error($mysqli));                           
    ...
    $pdf->Row(array("", "REQUESTED BY:", "APPROVED BY:","ISSUED BY:","RECEIVED BY:"));
	 ...
	 $status = mysqli_query($mysqli, "SELECT req_status from pms_ris where pms_ris.ris_no = $ris")
                                      or die("Error: ".mysqli_error($mysqli));                           
    $stat = mysqli_fetch_array($status);
	
	if ($stat['req_status']=='Approved') 
	{
    ...
    $pdf->Row(array("Printed Name", "{$name['P_fname']} {$name['P_lname']}", "Jeremy Cruz","Lloyd Garcia",""));
    $pdf->Row(array("Designation", "{$name['dept_name']}", "Principal's Office","Supply Office",""));
    $pdf->Row(array("Date", "{$var2}", "{$var2}","",""));
$pdf->Ln();
$pdf->output();
?>