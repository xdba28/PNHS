					  ...
					  <!-- Nav tabs -->
						...
						<br>
						  <!-- Tab panes -->
												...
												<br>
												<form action="issuance.php" method="post">&nbsp;&nbsp;
									            <input type="number" name="valueToSearch"  placeholder="Select PO #">
									            <input class="btn btn-primary btn" type="submit" name="search" value="Search"><br><br>
									            <div class="panel-heading">
  															
							
								<div align="right"><button onclick="printDiv('tbExport')" name="print" class="btn btn-outline btn-primary btn"><h2><i class="fa fa-print">  Print</i></h2></div>
...
if(isset($_POST['search']))
{
    $valueToSearch = $_POST['valueToSearch'];
            $qry="SELECT pims_personnel.P_lname,pims_personnel.P_fname FROM pms_po_con, pms_supplier, pms_pr_con, pms_ris_req, ims_storage, pms_ris, pms_pr, pms_po, pms_iar,pims_personnel, pms_iar_con WHERE pms_ris.ris_no=pms_ris_req.ris_no AND pms_ris_req.req_item_id=pms_pr_con.req_item_id AND pms_pr.pr_no=pms_pr_con.pr_no AND pms_supplier.company_id=pms_po.company_id AND pms_po.po_no=pms_po_con.po_no AND pms_po_con.pr_id=pms_pr_con.pr_id AND pms_iar.iar_no=pms_iar_con.iar_no AND pms_iar_con.po_id=pms_po_con.po_id AND ims_storage.emp_no=pims_personnel.emp_No AND pms_iar_con.iar_id=ims_storage.iar_id
            	AND pms_po.po_no=$valueToSearch ";
        ...
        <table>
            <tr>
        		...
        		<td width = "200"><center><?php echo date("Y-m-d") ?></center></th>
								...
								<a  class="btn btn-outline btn-primary btn" href='issuance.php'>Back to Top</a></center><br>
...
if(isset($_POST['print']))
{
  ...
  $sql1="INSERT INTO `ims_borrow`(`borrow_id`, `emp_no`, `stor_id`, `borrow_date`, `return_date`,`borrow_qty`,`trans_date`) VALUES ('$ins','$id','$quan','$loc',NULL,'$code',NULL)";
