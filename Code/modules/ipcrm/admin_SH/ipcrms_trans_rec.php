											...
											<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
												...
												<tbody>
													<?php
														...
														AND ipcrms_content.emp_No = pims_personnel.emp_No and Status='Verified'
														");
														$count = 1;
														while($pend = mysqli_fetch_assoc($pending))
														{
															...
															<td><div class="margin" align="center">
																<div class="btn-group">
																		<a href="view.php?emp=<?php echo $pend['emp_No']; ?>"> 
															...
															<?php 
																if($pend['Status']=="Verified"){
															?>
																	<div class="btn-group">
																		<input type="hidden" name="emp<?php echo $count; ?>" value="<?php echo $pend['emp_No']; ?>">
																		<button name="ver<?php echo $count; ?>" type="submit" class="btn btn-primary">Approve</button>
																	</div>
																	</div></td>
															<?php	
																}else{ ?>
													...
													<?php 
														if(isset($_POST['ver'.$count])){
															$emp=$_POST['emp'.$count];
															$upd=mysqli_query($mysqli,"UPDATE ipcrms_content SET Status='Approved' WHERE emp_no=$emp ");
															if($upd){
																echo "<script>alert('Form Approved!'); window.location.href='ipcrms_trans_rec.php';</script>";
															}else{
																echo "<script>alert('Error! Form not Approved!'); window.location.href='ipcrms_trans_rec.php';</script>";
															}
														}
														$count++;
