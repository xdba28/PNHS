...
<html lang="en" >
    ...
    if(isset($_SESSION['user_data']['priv']['ipcrms_priv']) && isset($_SESSION['user_data']['acct']['emp_no']) && isset($_SESSION['user_data']['acct']['cms_account_ID']) && isset($_SESSION['user_data']['acct']['cms_username']) && $_SESSION['user_data']['priv']['ipcrms_priv']=="1"){
		$emp=$_SESSION['user_data']['acct']['emp_no'];
        $id=$_SESSION['user_data']['acct']['cms_account_ID'];
        $rec=mysqli_query($mysqli,"SELECT dms_receiver.rec_no FROM dms_receiver,cms_account WHERE cms_account.cms_account_id=dms_receiver.cms_account_id AND dms_receiver.cms_account_id=$id");
        ...
        <li><a href="update_perf_indi.php">PERFORMANCE INDICATOR </a></li>
			...
			<div id="page-wrapper">
				<div class="container-fluid">
					...
					<div class="row">
						<div class="col-lg-12">
							<h3 class="page-header">Update Weight per KRA</h3> 
						</div>
					</div>
					<div class="col-lg-8">
						<h3>MFO:</h3>
						<form action="up_weight.php" method="get">
						<select class="form-control" name="mfo_id">
							<option value="" selected>-Select MFO-</option>
							<?php
								...
								if($qry){
									while($row=mysql_fetch_array($qry)){
										...
										<option value="'.$mfo_id.'">'.$mfo_desc.'</option>';
									}
								}
							?>
