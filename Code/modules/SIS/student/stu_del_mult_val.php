    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                $get_stu_name = mysql_query("SELECT lrn, stu_lname, stu_fname, stu_mname FROM sis_student WHERE lrn = $lrn")
                                        or die("Error get_stu_name: " .mysql_error());

                    $res = mysql_fetch_assoc($get_stu_name);
                    $id = $res['lrn'];
                    $last = $res['stu_lname'];
                    $first = $res['stu_fname'];
                    $middle = $res['stu_mname'];

                    $get_section = mysql_query("SELECT SECTION_NAME, YEAR_LEVEL, year FROM css_section, sis_stu_rec, css_school_yr
                    ...
                    $res = mysql_fetch_assoc($get_section);
                    $lvl = $res['YEAR_LEVEL'];
                    $sec = $res['SECTION_NAME'];
                    $sy = $res['year'];

                    $stu_arr = array($id, $sec);
                    $stu_imp = implode("-", $stu_arr);

                    echo 
                    '<td><input form="val_form" type="hidden" name="lrn[]" id="lrn[]" value=' . $stu_imp . '>' . $id . '</td>
                    <td>' . $last . '</td>
                    <td>' . $first . '</td>
                    <td>' . $middle . '</td>';
                    ...
                    ?>
                        ...
                        <form action="process/del_stu_mult.php" method="post" enctype="mutipart/form-daya" id="val_form">
                        <button form="val_form" type="submit" class="btn w3-hover-red btn-danger w3-card-2">
                        <i class="fa fa-trash-o"></i>&nbsp;&nbsp;Delete Selected&nbsp;
                        </button>
                        
                        <a href="stu_del_mult.php">
