...
//        $sec_id=13;
        ...
        //FEMALE
        $sql="SELECT COUNT(sis_gender) as Female FROM sis_student,sis_stu_rec,css_section,css_school_yr,sis_grade
         WHERE sis_student.lrn=sis_stu_rec.lrn 
         and sis_stu_rec.section_id=css_section.SECTION_ID 
         and sis_stu_rec.sy_id=css_school_yr.sy_id 
         and sis_stu_rec.rec_id=sis_grade.rec_id 
...
$sql="SELECT * FROM sis_student,sis_stu_rec,css_section,css_school_yr,sis_grade
         WHERE sis_student.lrn=sis_stu_rec.lrn 
         and sis_stu_rec.section_id=css_section.SECTION_ID 
         and sis_stu_rec.sy_id=css_school_yr.sy_id 
         and sis_stu_rec.rec_id=sis_grade.rec_id 
    ...
    while($row=mysqli_fetch_assoc($result))
    {
        ...
        if($row['grade_remarks'] == "Passed"){
            ...
            $remarks='RETAINED';
            $fail_q=mysqli_query($mysqli,"SELECT subject_name 
            FROM css_subject,sis_stu_rec,sis_student,sis_grade 
            WHERE sis_student.lrn=sis_stu_rec.lrn 
            AND sis_stu_rec.rec_id=sis_grade.rec_id
            ...
            $f_sub="";
            while($fr=mysqli_fetch_assoc($fail_q)){
                $f_sub.=" ".$fr['subject_name'];
            }
        }
        ...
        $pdf->Cell(23,4,"$grade",1,0,'C',false);
        $pdf->Cell(31,4,"$remarks",1,0,'C',false);
        if($remarks=="RETAINED"){
            $pdf->CellFitScale(33,4,"$f_sub",1,0,'C',false);
        }else{
            $pdf->Cell(33,4,'',1,0,'C',false);
        }
        $pdf->Ln();
...
$name=$per['Name']."_SchoolFileFive.pdf";
$pdf->Output('I',$name);
?>