<?php
	...
	while ( $row = mysqli_fetch_array($result) ){
		...
		if ( $row['p_update_status'] == "Pending" ){
              $echo2 = $echo2 . " $('#tbody1').append($('<tr>".$echo."</tr>').attr({ 'class': 'info' , 'id':'tr".$row['p_update_id']."' , 'onclick':'viewDetails(".$row['p_update_id'].")' })); ";
		      
        }
        else if ( $row['p_update_status'] == "Approved" ){
              $echo2 = $echo2 . " $('#tbody1').append($('<tr>".$echo."</tr>').attr({  'id':'tr".$row['p_update_id']."' , 'onclick':'viewDetails(".$row['p_update_id'].")' })); ";
		      
        }
        else if ( $row['p_update_status'] == "Disapproved" ){
              $echo2 = $echo2 . " $('#tbody1').append($('<tr>".$echo."</tr>').attr({ 'class': 'danger' , 'id':'tr".$row['p_update_id']."' , 'onclick':'viewDetails(".$row['p_update_id'].")' })); ";
