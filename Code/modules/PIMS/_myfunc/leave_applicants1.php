<?php
	...
	$query = "select * from pims_leave , pims_leave_balance , pims_personnel where leave_application_status = 'Pending' and pims_leave.leave_bal_id = pims_leave_balance.leave_bal_ID and pims_leave_balance.emp_No = pims_personnel.emp_No; ";
	$result = mysqli_query($_SESSION['pims_data']['connection'] , $query);
	
	$count = 1;
	
	while( $row = mysqli_fetch_array($result) ){
		$echo = $echo . " $('#tbody1').append($('<tr></tr>').attr({ 'class' : 'info' , 'id' : 'tr".$count."' })); ";
		...
		$echo = $echo . " $('#tr".$count."').append($('<td>".$row['place_to_be_visited']."</td>')); ";
		$echo = $echo . " $('#tr".$count."').append($('<td>".$row['leave_application_status']."</td>')); ";
