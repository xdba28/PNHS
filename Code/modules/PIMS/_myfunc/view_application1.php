<?php
	...
	while( $row = mysqli_fetch_array($result) ){
		...
		if ( $row['emp_No'] != $_SESSION['pims_data']['emp_id'] ){
			$echo = $echo . " $('#span10').append( '&emsp;' ); ";
			$echo = $echo . " $('#span10').append( $('<button>MESSAGE</button>').attr({ 'id':'messageButton' , 'type':'button' , 'class':'btn btn-outline btn-info' , 'onclick':'clickMessage(".$row['emp_No'].");' }) ); ";
		}
