<?php
	...
	$echo = $echo . " $('#zeroresults1').append($('<label>No results. Please try again</label>').attr({ 'style':'max-height: 10px; ' })); ";
	while ( $row2 = mysqli_fetch_array($result2) ){
		...
		$echo = $echo . " $('#td".$count."').append($('<a></a>').attr({ 'id':'a".$count."' , 'href':'browse_faculty.php?id=".$row2['emp_No']."' })); ";
