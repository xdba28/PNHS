...
include("../../curr.php");
$emp_no=$_GET['emp'];
$person=$mysqli->query("SELECT concat(p_lname,'_',p_fname) as Name, concat(p_fname,' ',p_lname) as arch
FROM pims_personnel
WHERE emp_no=$emp_no");
$per=$person->fetch_assoc();
$name=$per['Name']."_ServiceRecord.pdf";
$folder=$per['arch'];
...
$pdf->AddPage();
 ...
 while($row=mysqli_fetch_assoc($res)){                     
   ...
   $pdf->CellFitScale(19.7,5,"$sr_status",1,0,"C");
...
echo "<script>window.location.href='archive_pds.php?emp=$emp_no&disk=$disk';</script>";
?>
