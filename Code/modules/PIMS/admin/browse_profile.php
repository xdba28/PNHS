        ...
        <div class="container">
            ...
            <div class="panel-body">
              <div class="row">
                ...
                <?php include("../myfunc/db_connect.php"); $query = "select * from cms_account where emp_No = " . $_GET["id"] ; $result = mysqli_query($_SESSION['pims_data']['connection'],$query); $row = mysqli_fetch_array($result); if ($_SESSION['pims_data']['emp_id'] != $_GET['id']) echo '<a href="../messaging/chatroom.php?fr=2&id='.$row["cms_account_ID"].'">'; else echo "<a>"; mysqli_close( $_SESSION['pims_data']['connection'] ); unset( $_SESSION['pims_data']['connection'] ); ?><button class="btn btn-outline btn-primary">Message</button></a> <?php echo '<a href="update_personnel.php?id='.$_GET["id"].'" >'; ?><button class="btn btn-outline btn-primary">Update Profile</button></a></center>
                </div>
