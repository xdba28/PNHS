...
<?php 
    ...
    $queryreport = mysqli_query($connection,"SELECT * FROM prs_report WHERE month(date_issued) = '$prevmonth' AND year(date_issued) = '$prevyear'" );
    $checkreport = mysqli_num_rows($queryreport);

    $checkmtrmonth = mysqli_query($connection,"SELECT * FROM `prs_mtr` WHERE month(time_issued)= month(now()) and year(time_issued) =year(now())");
    $checkmtrrow = mysqli_num_rows($checkmtrmonth);


     $queryempselect= mysqli_query($connection," SELECT * FROM pims_personnel, pims_employment_records 
    Where pims_personnel.emp_No=pims_employment_records.emp_No");
     $checksalrec= '';
     $checkaddinfoget= '';

    $checkempno= mysqli_query($connection,"SELECT emp_No
                                        FROM pims_personnel 
                                        WHERE emp_No NOT IN
                                    (SELECT emp_No FROM prs_salary_record)");
    $rowcheckempno = mysqli_num_rows($checkempno);

    $checkempadd= mysqli_query($connection,"SELECT emp_No
                                        FROM pims_personnel 
                                        WHERE emp_No NOT IN
                    ...
                    <div class="col-md-10">
                        ...
                        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                            ...
                            <ul class="nav navbar-nav navbar-right">
                                ...
                                <?php
                                            if($dms_priv=="1" || $ipcr_priv=="1" || $prs_priv=="1" || $pms_priv=="1"){ ?>
