	...
	if ( $_GET['s'] == 1 ){
		...
		while ( $row = mysqli_fetch_array($result) ){
			if ( $row['p_column_name'] == 'P_picture' ){
				...
				else if ( $row['p_update_table'] == '4' ){
					$query1 = " insert into pims_work_experience set ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' ";
					$row = mysqli_fetch_array($result);
					$query1 = $query1 . " , ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' ";
					$row = mysqli_fetch_array($result);
					$query1 = $query1 . " , ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' ";
					$row = mysqli_fetch_array($result);
					$query1 = $query1 . " , ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' ";
					$query1 = $query1 . " , emp_No = ".$row['emp_No']." ";
				}
				else if ( $row['p_update_table'] == '5' ){
					$query1 = " insert into pims_training_programs set ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' ";
					$row = mysqli_fetch_array($result);
					$query1 = $query1 . " , ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' ";
					$row = mysqli_fetch_array($result);
					$query1 = $query1 . " , ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' ";
				...
				else if ( $row['p_update_table'] == '6' ){
					if ( $row['p_data_column'] == "p_data_int" ){
						$query1 = " update pims_employment_records set ".$row['p_column_name']." = ".$row[$row['p_data_column']]." where emp_No = ".$row['emp_No']." ";
					}
					else{
						$query1 = " update pims_employment_records set ".$row['p_column_name']." = '".$row[$row['p_data_column']]."' where emp_No = ".$row['emp_No']." ";
					}
