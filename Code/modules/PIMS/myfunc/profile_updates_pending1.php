<?php
	...
	$query = "select * from pims_profile_updates , pims_personnel where pims_personnel.emp_No = pims_profile_updates.emp_No and p_update_status = 'Pending' order by p_update_id desc; ";
	$result = mysqli_query( $_SESSION['pims_data']['connection'] , $query );
	$count = 1;
	while ( $row = mysqli_fetch_array($result) ){
		$echo = $echo . " $('#tbody1').append($('<tr></tr>').attr({ 'id':'tr".$row['p_update_id']."' , 'class':'info' , 'onclick':'loadSideMenu(".$row['emp_No'].",".$row['p_update_id'].")' })); ";
