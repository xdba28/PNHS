<?php
			...
			while($row = mysqli_fetch_array( $result )){
				...
				if ( !empty($row['training_title']) ){
					if ( mysqli_num_rows($result) > 1 ){
						$echo = $echo . " $('#trainingProgramsnv').append('<div class= col-md-12  >".$row['training_title']."</div>'); ";
					}
					else{
						$echo = $echo . " $('#trainingProgramsnv').append('<div class= col-md-12  >".$count1.". ".$row['training_title']."</div>'); ";
					}
				}
				else{
					if ( mysqli_num_rows($result) > 1 ){
						$echo = $echo . " $('#trainingProgramsnv').append('<div class= col-md-12  >Training @ ".$row['location']."</div>'); ";
					}
					else{
						$echo = $echo . " $('#trainingProgramsnv').append('<div class= col-md-12  >".$count1.". Training @ ".$row['location']."</div>'); ";
					}
				}
				...
				if ( $count1 > 5 ){
					$echo = $echo . " $('#trainingProgramsnv').append('<div class= col-md-12  >Click See All Training to see all.</div>'); ";
					break;
				}
			}
		}
	...
	function getpersonnelinfo(){
		...
		*/
		...
		$toBeEcho1 = $toBeEcho1 . "
			$('#height_m1').text('Height: ".$row['height_m']."');
			$('#weight_kg1').text('Weight: ".$row['weight_kg']."');
			$('#bloodtype1').text('Blood Type: ".$row['bloodtype']."');
			$('#GSIS_No1').text('GSIS ID No.: ".$row['GSIS_No']."');
			$('#PAG_IBIG_id1').text('PAGIBIG ID No.: ".$row['PAG_IBIG_id']."');
			$('#PHILHEALTH_no1').text('PHILHEALTH No.:  ".$row['PHILHEALTH_no']."');
			$('#SSS_no1').text('SSS No.: ".$row['SSS_no']."');
			$('#TIN_no1').text('TIN No.: ".$row['TIN_no']."');
			";
