...
if(!empty($_POST['time_e'])){
 ...
 //                              	AND pims_employment_records.emp_No=pims_personnel.emp_No
	...
	foreach ($teacher as $keys) {
		$emp_emp = $keys['emp_rec_ID'];
		$asss = mysqli_query($conn, "SELECT css_schedule.emp_rec_id FROM css_schedule, css_school_yr 
											WHERE css_school_yr.sy_id=css_schedule.sy_id AND status='used' 
										   AND emp_rec_ID=$emp_emp");
			
			$ll = mysqli_num_rows($asss);
			if($ll==8){
				$t_id[] = $emp_emp;
			}
		
	}
	$max = mysqli_query($conn, "SELECT * FROM css_loads");
	foreach ($max as $key) {
			$id_max = $key['emp_rec_id'];
			$loads = $key['max_load'];
			$ass = mysqli_query($conn, "SELECT css_schedule.emp_rec_id FROM css_schedule, css_school_yr 
											WHERE css_school_yr.sy_id=css_schedule.sy_id AND status='used' 
										   AND emp_rec_ID=$id_max");
			$l = mysqli_num_rows($ass);
			if($l==$loads){
				$t_id[] = $id_max;
			}
		}
