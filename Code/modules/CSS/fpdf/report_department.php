...
$pdf = new PDF_MC_Table('L','mm','Legal');
    ...
    for ($i=0; $i < count($times); $i++) { 
	   	$am[] = substr($times[$i], 0, -3)."-".substr($timee[$i], 0, -3); 
	}
...
foreach ($dept as $key) {
	...
	$sched = mysqli_query($conn, "SELECT SECTION_NAME, YEAR_LEVEL, TIME_START, TIME_END, subject_id, DAY
...
for($c=0; $c < $count; $c++){
	...
	$pdf->SetTitle('By Department Schedule S.Y. '.$row[0].'');
...
for($c=0; $c < $count; $c++){
	...
	for ($i=count($times)/2; $i < count($am); $i++) { 
