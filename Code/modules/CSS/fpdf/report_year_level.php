...
*/	$times = null;
	...
	foreach ($sec as $key) {
		...
		if($check==1){
		    ...
		    $tee=substr($rows['TIME_END'], 0, -3);
		      ...
		      $sched_p[$c][count($times)][$x]="";

		    if($rows['DAY']=='regular'){
      			if($sub==50005||$sub==50009||$sub==50012){
        			$sched_p[$c][count($times)][$x] .= $rows['subject_name'].' '.$teach_str."\n".$tss."-".$tee;
      			}
      			else{
        			$sched_p[$c][count($times)][$x] .= $rows['subject_name'].'/'.$teach_str."\n".$tss."-".$tee;
      			}
      			...
      			if($sub==50005||$sub==50009||$sub==50012){
        			$sched_p[$c][count($times)][$x] .= $rows['subject_name'].' '.$teach_str.'('.$rows['DAY'].')'."\n".$tss."-".$tee;
      			}
      			else{
        			$sched_p[$c][count($times)][$x] .= $rows['subject_name'].'/'.$teach_str.'('.$rows['DAY'].')'."\n".$tss."-".$tee;
      			}
	...
	$pdf->SetTitle('Year Level Class Schedule S.Y. '.$row[0].'');
