...
$_SESSION['grade'] = 7;
$grade = 7;
$section = mysqli_query($conn, "SELECT SECTION_ID, SECTION_NAME, css_school_yr.SY_ID FROM css_section, css_school_yr 
                                WHERE status='used' AND css_section.SY_ID=css_school_yr.SY_ID 
                                AND YEAR_LEVEL=7");

if(mysqli_num_rows($section)==0){
  $sy_sql = mysqli_query($conn, "SELECT sy_id FROM css_school_yr");
  $comf = mysqli_fetch_row($sy_sql);
  $yr_id = $comf[0];
}
else{
  $comf = mysqli_fetch_row($section); 
  $yr_id = $comf[2];
}
...
//check adviser
  ...
  $query = mysqli_query($conn, "SELECT SECTION_ID FROM css_section, css_school_yr WHERE css_section.SY_ID=css_school_yr.SY_ID AND status='used' AND emp_rec_id!=null");
