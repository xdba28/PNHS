    ...
    if(!empty($_POST['sub'])){
        ...
        if(!empty($_POST['day'])){
            ...
            $query = mysqli_query($conn, "SELECT * FROM css_schedule, css_school_yr  
                                        ...
                                        AND css_schedule.SY_ID=css_school_yr.SY_ID
                                        AND SECTION_ID = $sec
                                        AND status = 'used' AND TIME_START='$ts' AND TIME_END='$te'
                                        ");
            $query1 = mysqli_query($conn, "UNLOCK TABLES");
            ...
            if($rows!=0){
                ...
                if($count!=0){
                    echo ("<SCRIPT LANGUAGE='JavaScript'>
                            window.alert('Sorry, (". substr($day_error, 0, -1).") is/are not available in selected time.')
                ...
                if(count($teach)==1){
                    ...
                    if($day=='regular'){
                        $query = mysqli_query($conn, "SELECT * FROM css_schedule, css_school_yr
                                        ...
                                        AND status = 'used' AND (DAY='regular' OR DAY!='regular')");
                        $rows = mysqli_num_rows($query);
                        if($rows!=0){
                            echo ("<SCRIPT LANGUAGE='JavaScript'>
                        ...
                        $query = mysqli_query($conn, "SELECT * FROM css_schedule, css_school_yr
                                        ...
                                        AND status = 'used' AND emp_rec_ID=$teach
                                        AND DAY!='regular' ");
                        $rows = mysqli_num_rows($query);
                        if($rows!=0){
                            foreach ($query as $key) {
