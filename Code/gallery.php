...
<html lang="en">
	<head>
		...
		<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no">
	  	<title>Gallery | PAG-ASA National High School</title>
		...
		<meta property="og:title" content="Gallery | PAG-ASA National High School" />
		<meta property="og:site_name" content="Gallery | PAG-ASA National High School" />
	  	<meta name="description" content="Gallery | PAG-ASA National High School">
		...
		<link href="Template (reference)/dist/css/sb-admin-2.css" rel="stylesheet">
				...
				<div class="col-lg-8">
					...
					<?php
										...
										if(isset($_GET['q'])) {
											...
											$result_num_rows_query = "SELECT * FROM `cms_album` WHERE `gallery_type` = 'documentation' ORDER BY `cms_album_date_created` DESC, `cms_album_time_created` DESC;";
										}
										else {
											$result_num_rows_query="SELECT * FROM `cms_album` WHERE `gallery_type` = 'documentation' ORDER BY `cms_album_date_created` DESC, `cms_album_time_created` DESC;";
										}
					...
					<div class="row">
						<?php
							$doc_query="SELECT * FROM `cms_album` WHERE `gallery_type` = 'documentation' ORDER BY `cms_album_date_created` DESC, `cms_album_time_created` DESC LIMIT ".$this_page_num_result.", ".$result_num.";";
								...
								while($doc = $get_doc->fetch_assoc()) {
									...
									if($gallery_type == 'documentation') {
										...
										while($img = $get_img->fetch_assoc()) {
											...
											$id2 = base64_encode($img['cms_album_ID']);
										...
										echo '<div class="col-lg-4">
														<div class="thumbnail">
															<a href="gallery_view.php?id='.$idbase.'">';
										if(!empty($dir2)) {
															echo '	<img src="uploads/docu/'.$alfol.'/'.$name2.'" class="img-padding img-responsive center-block" style="height: 200px; width: auto;">';
														}
														else {
										...
										echo      '</a>
															<div class="caption text-center">';
																				if(strlen($name) > 15) {
																					echo '<small title="'.$name.'" class=""><b>'.trim(substr(strip_tags($name, '<p></p>'), 0, 15)).'...</b></small>';
																				}
																				else {
