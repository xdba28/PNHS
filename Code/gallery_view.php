...
<html lang="en" >
	<head>
		...
		<?php
				$linkid = $mysqli->real_escape_string($_GET['id']);
				$getid = base64_decode($mysqli->real_escape_string($_GET['id']));
				if(is_numeric($getid)) {
					$title_query="SELECT * FROM `cms_album` WHERE `cms_album_ID`='$getid';";
					$get_title = $mysqli->query($title_query);
					if($get_title->num_rows > 0) {
						while($ttle = $get_title->fetch_assoc()) {
							$intitle = $ttle['cms_album_name'];
							$indate = $ttle['cms_album_date_created'];
							$indesc = $ttle['cms_album_desc'];
							$alfol = $ttle['cms_album_folder'];
						}
					}
				}
			?>
	  	<title><?php echo $intitle.' | '; ?>PAG-ASA National High School</title>
		...
		<meta property="og:title" content="<?php echo $intitle.' | '; ?>PAG-ASA National High School" />
		<meta property="og:site_name" content="<?php echo $intitle.' | '; ?>PAG-ASA National High School" />
	  	<meta name="description" content="<?php echo $intitle.' | '; ?>PAG-ASA National High School">
        ...
        <div id="wrapper"> 
			...
			<div id="main" class="container-fluid">
				...
				echo '<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
									...
									<form action="php/_login.php" method="POST">
										<div class="modal-body">';
												...
												if(isset($_SESSION['namu']) AND isset($_SESSION['pasu'])) {
													echo '<p id="pe" class="text-danger">Username or Password is Incorrect</p>';
												}
										echo '</div>
						...
						unset($_SESSION['namu']); unset($_SESSION['pasu']); unset($_SESSION['msg']);
			...
			<ol class="breadcrumb">
				...
				<li class="active"><?php if(isset($intitle)) { echo $intitle; } ?></li>
			</ol>
			<div class="row">
				<div class="col-lg-8">
					...
					<div class="row">
						<div class="col-lg-12 text-justify">
							<h1><b><?php if(isset($intitle)) { echo $intitle; } ?></b></h1>
								<p><i><?php if(isset($indate)) { echo dateFormat($indate); } ?></i></p>
								<p><?php if(isset($indesc)) { echo $indesc; } ?></p>
								<hr>
								<?php
									$getid = base64_decode($mysqli->real_escape_string($_GET['id']));
										$result_num = 12;
												  ...
												  <ul class="pagination">
												    <?php
												    	...
												    	for($ite=1; $ite <= $page_num; ++$ite, $act='') {
												    		...
												    		if(isset($_GET['q'])) {
												    			echo '<li'.$act.'><a href="?id='.$linkid.'&q='.$mysqli->real_escape_string($_GET['q']).'&page='.$ite.'">'.$ite.'</a></li>';
												    		}
												    		else {
												    			echo '<li'.$act.'><a href="?id='.$linkid.'&q='.$qq.'&page='.$ite.'">'.$ite.'</a></li>';
												    		}
												    	}
												    ?>
												  </ul>
									...
									if(is_numeric($getid)) {
										...
										if($get_doc->num_rows > 0) {
											...
											while($doc = $get_doc->fetch_assoc()) {
												...
												$date = $doc['cms_image_date'];
												$cap = $doc['cms_img_cap'];
												$alb = $doc['cms_album_name'];
												echo '<div class="col-lg-4 img-thumb">
																			<div class="thumbnail">
																				<div class="row">
																					<div class="col-lg-12">
																						<a class="text-center" href="uploads/'.$dir.'/'.$alfol.'/'.$name.'" data-lightbox="'.$intitle.'" data-title="'.$cap.'">
																							<img src="uploads/'.$dir.'/'.$alfol.'/'.$name.'" alt="" class="img-padding img-responsive center-block text-center" style="height: 200px; width: auto;">
																						</a>
																						<div class="caption text-center">
																							<small class="text-primary"><b>'.dateFormat($date).'</b></small>
																							<br>
																						</div>
																					</div>
																				</div>
											...
											echo '<div class="col-lg-12"><div class="alert alert-info" role="alert">
