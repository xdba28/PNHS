...
<html lang="en" >
	<head>
		...
		<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no">
	  	<title>Memo | PAG-ASA National High School</title>
		...
		<meta property="og:title" content="Memo | PAG-ASA National High School" />
		<meta property="og:site_name" content="Memo | PAG-ASA National High School" />
	  	<meta name="description" content="Memo | PAG-ASA National High School">
		...
		<link href="js/_temp/dist/css/sb-admin-2.css" rel="stylesheet">
		<link href="js/_temp/temp/datatables-plugins/dataTables.bootstrap.css" rel="stylesheet">
		<link href="js/_temp/temp/datatables-responsive/dataTables.responsive.css" rel="stylesheet">
        ...
        <div id="wrapper"> 
			...
			<div class="row">
				<div class="col-lg-8">
					...
					<br>
					<div class="row">
										<form action="admin/_Sub/ss_memo_idx.php" method="GET">
								...
								<div class="panel-body">
									<?php
										...
										if(isset($_GET['q'])) {
											...
											$result_num_rows_query = "SELECT * FROM `cms_memo` WHERE `cms_subject` LIKE '%$search%' OR `cms_memo_description` LIKE '%$search%' OR `cms_memo_date` LIKE '%$search%' ORDER BY `cms_memo_date` DESC;";
										}
										else {
											$result_num_rows_query="SELECT * FROM `cms_memo`;";
										}
										...
										<?php
											if(isset($_GET['q'])) {
												if(isset($_GET['q']) AND !empty($_GET['q'])) {
													$search = $mysqli->real_escape_string($_GET['q']);
													$news_query="SELECT * FROM `cms_memo` WHERE `cms_subject` LIKE '%$search%' OR `cms_memo_description` LIKE '%$search%' OR `cms_memo_date` LIKE '%$search%' ORDER BY `cms_memo_date` DESC LIMIT ".$this_page_num_result.", ".$result_num.";";
													$get_news = $mysqli->query($news_query);
													if($get_news->num_rows > 0) {
														while($news = $get_news->fetch_assoc()) {
															$title = $news['cms_subject'];
															$desc = $news['cms_memo_description'];
															$date = $news['cms_memo_date'];
															$id = $news['cms_memo_ID'];
															$img = $news['cms_memo_img_dir'];
															$pdf = $news['cms_memo_pdf_dir'];
															echo '<tr>';
															echo '<td>
																			<div class="media">
																				<div class="media-left">
																					<a href="memo_view.php?id='.base64_encode($id).'" id="mediatable">
																					</a>
																				</div>
														...
														echo '<tr><td class="info">No Match Found.</td></tr>';
													}
												}
												else {
													$news_query="SELECT * FROM `cms_memo` ORDER BY `cms_memo_date` DESC LIMIT ".$this_page_num_result.", ".$result_num.";";
													$get_news = $mysqli->query($news_query);
													if($get_news->num_rows > 0) {
														while($news = $get_news->fetch_assoc()) {
															$title = $news['cms_subject'];
