...
include('../connection.php');
 include('../sanitise.php');
		...
		$date = date('MY');
		$filenames =  'PayrollSlip'.$date.'.pdf';
		$check=mysqli_query($mysqli,"select * from prs_save_report   where prs_save_report.filename='$filenames';");
	...
	readfile($path);
        ...
        $qry = mysqli_query($mysqli,"SELECT * FROM pims_personnel, prs_report , prs_salary_record, prs_grade, prs_salary, pims_employment_records, pims_job_title, prs_allowance
...
<tr>
<td colspan="4" style="font-size:12px; " align="center"><h3>PAYROLL SLIP</h3></td>
</tr>
...
';
 ...
 $loancheck = mysqli_query($mysqli,"SELECT * FROM pims_personnel, prs_loan_pay
...
while($row1=mysqli_fetch_assoc($loancheck))
										{       									
...
if (@file_exists(dirname(__FILE__).'/lang/eng.php')) {
	...
	$check=mysqli_query($mysqli,"select * from prs_save_report   where prs_save_report.filename='$filename';");
    $checkrows=mysqli_num_rows($check);
   ...
   if($checkrows>0) {
      ...
      $query = mysqli_query($mysqli,"INSERT prs_save_report(filename, file_location, type ) values('$filename','$save2', 'Slip')");
	      }
