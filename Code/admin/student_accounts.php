							...
							<div class="panel panel-primary sidepanel">
								<div class="panel-body">
									...
									<div class="row">
										...
										<br class="hidden-lg">
										<br class="hidden-lg">
										<form action="stu_acct.php" method="GET" target="_blank">
											<div class="col-xs-8 col-sm-8 col-lg-4">
										...
										<div class="panel-body">
											<?php
												...
												if(isset($_GET['q'])) {
													...
													$result_num_rows_query = "SELECT `cms_account`.`cms_account_ID`, `cms_account`.`lrn`, `cms_account`.`status`, `stu_fname`, `stu_mname`, `stu_lname`, `year_level`, `section_name`, `year` FROM `cms_account`, `sis_student`, `css_school_yr`, `sis_stu_rec`, `css_section` WHERE (`stu_fname` LIKE '%$search%' OR `stu_mname` LIKE '%$search%' OR `stu_lname` LIKE '%$search%' OR CONCAT(`year_level`, '-', `section_name`) LIKE '%$search%') AND `cms_account`.`lrn` = `sis_student`.`lrn` AND sis_student.lrn = sis_stu_rec.lrn AND sis_stu_rec.sy_id = css_school_yr.sy_id AND sis_stu_rec.section_id = css_section.SECTION_ID AND sis_stu_rec.section_id IS NOT NULL ORDER BY `stu_lname` ASC, `stu_fname` ASC, `stu_mname` ASC;";
												}
												else {
														...
														if(isset($_GET['q']) AND !empty($_GET['q'])) {
															...
															if($get_sis->num_rows > 0) {
																...
																while($sis = $get_sis->fetch_assoc()) {
																	...
																	echo '<tr>';
																	echo '<td>'.$stu_lname.'</td>';
																	echo '<td>'.$stu_fname.'</td>';
																	echo '<td>'.$stu_mname.'</td>';
