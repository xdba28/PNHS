													...
													<table width="100%" class="table table-striped table-bordered table-hover">
														...
														<?php
															if(isset($_GET['q'])) {
																...
																$proj_query="SELECT * FROM `cms_online_users` ORDER BY `cms_online_users_lname` ASC;";
																$get_proj = $mysqli->query($proj_query);
																if($get_proj->num_rows > 0) {
																	while($proj = $get_proj->fetch_assoc()) {
																		$fname = $proj['cms_online_users_fname'];
																		$mname = $proj['cms_online_users_mname'];
																		$lname = $proj['cms_online_users_lname'];
																		echo '<tr>';
																			echo '<td>'.$fname.'</td>';
																			echo '<td>'.$mname.'</td>';
																			echo '<td>'.$lname.'</td>';
																	...
																	echo '<tr><td class="danger" colspan="3">No Online Users.</td></tr>';
																}
															}
														?>
														</tbody>
