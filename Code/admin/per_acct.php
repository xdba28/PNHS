<?php
    ...
    if($get_sec_lvl == 'All') {
        $get_section = $mysqli->query("SELECT pims_department.dept_name , pims_personnel.emp_No , pims_personnel.P_fname, pims_personnel.P_mname, pims_personnel.P_lname, cms_account.cms_username, cms_account.cms_password FROM pims_personnel, pims_employment_records, pims_department, cms_account WHERE pims_department.dept_ID=pims_employment_records.dept_ID AND pims_personnel.emp_No=pims_employment_records.emp_No AND pims_personnel.emp_No=cms_account.emp_no;");
    }
    else {
    ...
    $file_name = $get_sec_lvl.'Personnel Accounts.pdf';
    ...
    while($row = mysqli_fetch_array($get_section)) {
        ...
        $name = iconv('UTF-8', 'windows-1252', ($row['P_lname'] . ", " . $row['P_fname'] . " " . $row['P_mname']));
