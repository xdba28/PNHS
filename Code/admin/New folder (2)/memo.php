														...
														<?php
															if(isset($_GET['s']) AND isset($_GET['q'])) {
																if(isset($_GET['q']) AND !is_null($_GET['q']) AND !empty($_GET['q']) AND $_GET['q']!=="") {
																	$memo_query="SELECT * FROM `cms_memo`;";
																	...
																	while($memo = $get_memo->fetch_assoc()) {
																		...
																		$subj2 = strtolower($subj);
																		$desc2 = strtolower($desc);
																		$date2 = strtolower($date);
																		$reci2 = strtolower($recipient);
																		$search = strtolower($q);
																		if(strstr($subj2, $search) OR strstr($desc2, $search) OR strstr($date2, $search) OR strstr($reci2, $search)) {
																			echo '<tr>';
																			echo '<td>'.$date.'</td>';
																			echo '<td>'.$subj.'</td>';
