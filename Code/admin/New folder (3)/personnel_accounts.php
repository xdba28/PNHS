							...
							<div class="panel panel-primary sidepanel">
								<div class="panel-body">
									...
									<div class="row">
										...
										<form action="per_acct.php" method="GET">
											<div class="col-lg-4">
												...
												<?php
													if(isset($_GET['q'])) {
														if(isset($_GET['q']) AND !empty($_GET['q'])) {
															$search = $mysqli->real_escape_string($_GET['q']);
															$pims_query="SELECT `emp_no`, `P_fname`, `P_mname`, `P_lname` FROM `pims_personnel` WHERE `P_fname` LIKE '%$search%' OR `P_mname` LIKE '%$search%' OR `P_lname` LIKE '%$search%' ORDER BY `P_lname` ASC, `P_fname` ASC, `P_mname` ASC LIMIT ".$this_page_num_result.", ".$result_num.";";
															...
															if($get_pims->num_rows > 0) {
																while($pims = $get_pims->fetch_assoc()) {
																	...
																	$cms_query="SELECT `cms_account_ID`, `status` FROM `cms_account` WHERE `emp_no` = '$emp_no' LIMIT 1;";
															...
															$pims_query="SELECT `emp_no`, `P_fname`, `P_mname`, `P_lname` FROM `pims_personnel` ORDER BY `P_lname` ASC, `P_fname` ASC, `P_mname` ASC LIMIT ".$this_page_num_result.", ".$result_num.";";
															$get_pims = $mysqli->query($pims_query);
															while($pims = $get_pims->fetch_assoc()) {
																$P_lname = $pims['P_lname'];
																$P_fname = $pims['P_fname'];
																...
																echo '<td>'.$faculty_type.'</td>';
																//echo '<td class="text-center mark"><input class="check" type="checkbox" name="delete_paccounts[]" value="'.$emp_no.'"></td>';
																$cms_query="SELECT `cms_account_ID`, `status` FROM `cms_account` WHERE `emp_no` = '$emp_no' LIMIT ".$this_page_num_result.", ".$result_num.";";
																$get_cms = $mysqli->query($cms_query);
																$iemp = $istat = '';
																while($data = $get_cms->fetch_assoc()) {
																	$iemp = $data['cms_account_ID'];
