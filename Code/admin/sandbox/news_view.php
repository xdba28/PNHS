...
<html lang="en">
	<head>
		...
		<?php
			$newsid = base64_decode($mysqli->real_escape_string($_GET['id']));
			if(is_numeric($newsid)) {
				$news_query="SELECT * FROM `cms_news` WHERE `cms_news_ID` = ".$newsid." LIMIT 1;";
				$get_news = $mysqli->query($news_query);
				if($get_news->num_rows > 0) {
					while($news = $get_news->fetch_assoc()) {
							$titlemeta = $news['cms_title'];
							$descmeta = $news['cms_news_description'];
							$datemeta = $news['cms_news_date'];
							$authmeta = $news['cms_news_writer'];
							echo '<meta name="subject" content="'.$titlemeta.'">';
							echo '<meta name="description" content="'.trim(substr(html_entity_decode(strip_tags($descmeta)), 0, 120)).'...'.'">';
							echo '<meta name="author" content="'.$authmeta.'">';
						}
					}
				}
		?>
	  	<title><?php echo $titlemeta.' | '; ?>PAG-ASA National High School</title>
	  	<meta http-equiv="Content-Type" content="text/html">
		...
		<meta property="og:title" content="<?php echo $titlemeta.' | '; ?>PAG-ASA National High School" />
		<meta property="og:site_name" content="<?php echo $titlemeta.' | '; ?>PAG-ASA National High School" />
		<meta name="description" content="<?php echo $titlemeta.' | '; ?>PAG-ASA National High School">
	...
	<body onload="myFunction()">
        ...
        <div id="wrapper"> 
			...
			<ol class="breadcrumb">
				...
				<li class="active">
					<?php
						$title2 = $desc2 = $date2 = $id2 = $dir2 = $cap2 = $auth2 = '';
						$newsid = base64_decode($mysqli->real_escape_string($_GET['id']));
						if(is_numeric($newsid)) {
							$news_query="SELECT * FROM `cms_news` WHERE `cms_news_ID` = ".$newsid." LIMIT 1;";
							$get_news = $mysqli->query($news_query);
							if($get_news->num_rows > 0) {
								while($news = $get_news->fetch_assoc()) {
									$title2 = $news['cms_title'];
									$desc2 = $news['cms_news_description'];
									$date2 = $news['cms_news_date'];
									$id2 = $news['cms_news_ID'];
									$dir2 = $news['cms_news_img_dir'];
									$cap2 = $news['cms_news_img_cap'];
									$auth2 = $news['cms_news_writer'];
								}
							}
							if($get_news->num_rows > 0) {
								if(!empty($title2)) {
						...
						<div class="col-lg-12">
							<?php
								if(is_numeric($newsid)) {
									if($get_news->num_rows > 0) {
							?>
							<h1 style="font-family: Georgia, \"Times New Roman\", Times, serif;"><b><?php echo $title2; ?></b></h1>
							<p><i>By: <?php echo $auth2." - ".dateFormat($date2); ?></i></p>
									<hr>
									<div class="row">
											<div class="col-lg-12">
												<p style="font-family: Georgia, \"Times New Roman\", Times, serif;"><?php echo $desc2; ?></p>
												<br>
												<?php
													if(!empty($dir2)) {
														echo '<img src="uploads/'.$dir2.'" alt="" class="img-responsive center-block">
														<br>
														<p><i>'.$cap2.'</i></p>';
													}
												?>
