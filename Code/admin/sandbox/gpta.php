...
<html lang="en">
	<head>
		...
		<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no">
	  	<title>PNHS GPTA Officers | PAG-ASA National High School</title>
		...
		<meta property="og:title" content="PNHS GPTA Officers | PAG-ASA National High School" />
		<meta property="og:site_name" content="PNHS GPTA Officers | PAG-ASA National High School" />
	  	<meta name="description" content="PNHS GPTA Officers | PAG-ASA National High School">
	...
	<body onload="myFunction()">
        ...
        <div id="wrapper"> 
			...
			<ol class="breadcrumb">
				...
				<li class="active">PNHS GPTA Officers</li>
			</ol>
			<div class="row">
				<div class="col-lg-8">
					<div class="w3-container w3-theme-bd5 w3-card-2">
						<center><h2>PNHS GPTA Officers</h2></center>
					</div>
					<?php
										$result_num = 20;
										if(isset($_GET['q'])) {
					...
					<br>
					<div class="row">
										<form action="admin/_Sub/ss_gpta_idx.php" method="GET">
											<div class="col-lg-8">
													...
													<?php
														if(isset($_GET['q'])) {
															if(isset($_GET['q']) AND !empty($_GET['q'])) {
																$org_query="SELECT CONCAT ( `cms_gpta_year1`, \"-\", `cms_gpta_year2` ) AS SY, `cms_gpta_ID`, `cms_gpta_content` FROM `cms_gpta` WHERE CONCAT ( `cms_gpta_year1`, \"-\", `cms_gpta_year2` ) = '2017-2018' ORDER BY `cms_gpta`.`cms_gpta_year1` DESC;";
																$get_org = $mysqli->query($org_query);
																if($get_org->num_rows > 0) {
																	while($org = $get_org->fetch_assoc()) {
																		$conid = $org['cms_gpta_ID'];
																		$content = $org['cms_gpta_content'];
																...
																$org_query="SELECT * FROM `cms_gpta` ORDER BY `cms_gpta_ID` DESC;";
																$get_org = $mysqli->query($org_query);
																if($get_org->num_rows > 0) {
																	while($org = $get_org->fetch_assoc()) {
																		$conid = $org['cms_gpta_ID'];
