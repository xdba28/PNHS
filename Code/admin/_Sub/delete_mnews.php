<?php
	...
	if(isset($_SESSION['user_data'])) {
			...
			if(!isset($_POST['delete_news']) and empty($_POST['delete_news'])) {
				$_SESSION['msg']='Must Select Article.';
				header('Location: ../news.php');
			}
			else if(isset($_POST['s']) AND isset($_POST['delete_news']) AND is_array($_POST['delete_news'])) {
				...
				$mysqli->autocommit(TRUE);
				$num = count($idary);
				$_SESSION['msg']='Deleted '.$success_no.' Article(s)';
				header('Location: ../news.php');
			}
			else {
				$_SESSION['msg']='Failed to delete '.$success_no.' Article(s)';
				header('Location: ../news.php');
			}
	}
	else {
