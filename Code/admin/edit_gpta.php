						...
						<div class="col-lg-12">
							<div class="panel panel-primary sidepanel">
								<div class="panel-body">
									...
									<?php
		                            	if(isset($_GET['id'])) {
		                            		$gid = trim($mysqli->real_escape_string($_GET['id']));
		                            	}
		                            	$cont_query = "SELECT * FROM `cms_gpta` WHERE `cms_gpta_ID` = '$gid';";
								          $get_cont = $mysqli->query($cont_query);
								          while($cont = $get_cont->fetch_assoc()) {
								            $get_content = $cont['cms_gpta_content'];
								            $y1 = $cont['cms_gpta_year1'];
								            $y2 = $cont['cms_gpta_year2'];
								          }
		                            ?>
									<hr>
									<form id="register" action="_Sub/update_gpta.php" method="POST" enctype="multipart/form-data">
										<label for="title">S.Y.: </label>
											<div class="row">
												<div class="col-lg-3">
													<input type="text" class="form-control" id="sy1" placeholder="YYYY" name="sy1" minlength="4" maxlength="4" value="<?php echo $y1; ?>">
												</div>
												<div class="col-lg-3">
													<input class="form-control" type="text" id="sy2" placeholder="YYYY" name="sy2" minlength="4" maxlength="4" value="<?php echo $y2; ?>">
												</div>
											</div>
											<br>
		                            <div class="row">
		                              <div class="col-lg-12">
		                              	<label for="gpta">Content: </label>
		                                <textarea class="ckeditor form-control" name="gpta" id="gpta"> <?php echo $get_content; ?> </textarea>
		                                <?php
		                                	if(isset($_GET['id'])) {
		                                		$gid = trim($mysqli->real_escape_string($_GET['id']));
