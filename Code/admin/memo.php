	...
	<body onload="myFunction()">
						...
						<div class="col-lg-12">
							<div class="panel panel-primary sidepanel">
								<div class="panel-body">
									...
									<div class="row">
										<div class="col-lg-4">
											<a class="btn btn-primary form-control" href="add_memo.php">Add Memo</a>
										</div>
									</div>
									<hr>
									<div class="row">
										<form action="_Sub/ss_memo.php" method="GET">
									...
									<br>
									<form action="_Sub/delete_mmemo.php" method="POST">
									<div class="row">
											<div class="col-lg-4">
											</div>
											<div class="col-lg-4">
											...
											<div class="col-lg-4">
												<input type="submit" class="btn btn-danger form-control" name="s" value="Delete Selected Memorandum"></input>
											</div>
									</div>
									<br>
									<div class="row">
														...
														<?php
															if(isset($_GET['q'])) {
																if(isset($_GET['q']) AND !empty($_GET['q'])) {
																	$search = $mysqli->real_escape_string($_GET['q']);
																	$memo_query="SELECT * FROM `cms_memo` WHERE `cms_subject` LIKE '%$search%' OR `cms_memo_description` LIKE '%$search%' OR `cms_memo_date` LIKE '%$search%' ORDER BY `cms_memo_date` DESC LIMIT ".$this_page_num_result.", ".$result_num.";";
																	...
																	if($get_memo->num_rows > 0) {
																		while($memo = $get_memo->fetch_assoc()) {
																			...
																			echo '<td>'.$date.'</td>';
																			echo '<td>'.$subj.'</td>';
																			echo '<td class="text-center mark"><input class="check" type="checkbox" name="delete_memo[]" value="'.$id.'"></td>';
																			//echo '<td>'.substr(strip_tags($desc), 0, 35).'...'.'</td>';
																			echo '<td hidden="hidden">'.$subj.'</td>
																			<td hidden="hidden">'.$id.'</td>
																			<td hidden="hidden"></td>';
																			echo '<td class="info text-center">
																							<a href="view_memo.php?id='.$id.'"><span class="fa fa-edit"></span> view </a>
																						</td>';
																	...
																	$memo_query="SELECT * FROM `cms_memo` ORDER BY `cms_memo_date` DESC LIMIT ".$this_page_num_result.", ".$result_num.";";
																	$get_memo = $mysqli->query($memo_query);
																	while($memo = $get_memo->fetch_assoc()) {
																		$subj = $memo['cms_subject'];
																		$desc = $memo['cms_memo_description'];
		...
		<script>
			...
			$('#dataTables-example > tbody').on('click', 'tr > td > a.delete', function() {
				...
				var insert = 'Are you sure you want to delete this memo: <br>' + data[3] + "?";
				$("#del-link").attr("href", "_Sub/delete_memo.php?id=" + data[4]);
